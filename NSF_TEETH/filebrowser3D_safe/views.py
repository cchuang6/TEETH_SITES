...
from filebrowser3D_safe.settings import *
from filebrowser3D_safe.functions import (get_path, get_breadcrumbs,
    get_filterdate, get_settings_var, get_directory, convert_filename)
from filebrowser3D_safe.templatetags.fb_tags import query_helper
from filebrowser3D_safe.base import FileObject
from filebrowser3D_safe.decorators import flash_login_required
...
mixin_class_name = "filebrowser3D_safe.storage.%sMixin" % storage_class_name

# Workaround for django-s3-folder-storage
if django_settings.DEFAULT_FILE_STORAGE == 's3_folder_storage.s3.DefaultStorage':
    mixin_class_name = 'filebrowser3D_safe.storage.S3BotoStorageMixin'
...
def mkdir(request):
    ...
    from filebrowser3D_safe.forms import MakeDirForm
...
def rename(request):
    ...
    from filebrowser3D_safe.forms import RenameForm
...
def view3D(request):
    ...
    # print 'filebrowser.views.py: django_settings.MEDIA_URL',
    #  django_settings.MEDIA_URL
    # print "get_directory()", get_directory()
    file_url = os.path.join(django_settings.MEDIA_URL, get_directory(),path,filename).replace("\\", "/")
